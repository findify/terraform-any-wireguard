...
variable "clients" {
  type = list(object({
    ...
    route_ip_ranges = optional(string, "{route-ip-ranges}")    # comma separated IP ranges that the client should route via the VPN in format "1.2.3.4/32,1.1.1.1/16", set "0.0.0.0/0" to route all traffic through VPN
